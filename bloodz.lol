...
StarterGui:SetCore("SendNotification", {
    Title = "🩸",
    Text = "bloodz.lol is loading please wait some seconds",
...
StarterGui:SetCore("SendNotification", {
    Title = "🩸",
    Text = "This is beta version please except bugs and report them on discord",
    Duration = 8, -- seconds the notification stays on screen
    Button1 = "ok"
})
...
if script_key then
    writefile("bloodzlol_Key.txt", script_key)
end
...
if isfunctionhooked then
    if isfunctionhooked(FireServer) or isfunctionhooked(UnreliableFireServer) or isfunctionhooked(InvokeServer) and LPH_OBFUSCATED then
        return Services.LocalPlayer:Kick("bloodz.lol | Security : You are running another script, please disable it and execute again")
    end
end
...
getgenv().library = {
    directory = "bloodz.lol_remastered",
    folders = {
        "/fonts",
        "/configs",
        "/assets"
    ...
    LPH_JIT_MAX(function()
        ...
        if Game_Name == "South Bronx" then
            ...
            HideUI = LPH_NO_VIRTUALIZE(function(Title, Timing)
                ...
                textLabel.Text = '<font color="rgb(0,163,224)">bloodz.</font>lol\n' .. Title
...
local window = library:window({name = "bloodz", suffix = ".lol", gameInfo = string.format("bloodz.lol : %s", Game_Name:lower())})
